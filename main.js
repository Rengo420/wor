...
//cards switch
    ...
    function fadeIn(el) {
        ...
        setTimeout(() => el.classList.add("show"), 10); // triggers fade
    }
    ...
    // Show products and hide main
    productsBtn.addEventListener("click", function (e) {
        e.preventDefault();
        fadeOut(mainContent);
        fadeIn(productsSection);
        productsSection.scrollIntoView({ behavior: "smooth" });
    });

    // All other nav links
    document.querySelectorAll(".header-nav__links a").forEach(link => {
        if (link !== productsBtn) {
            link.addEventListener("click", function (e) {
                ...
                // Switch view back to main content
                fadeOut(productsSection);
                fadeIn(mainContent);
    ...
    // Ensure products are hidden on page load
